...
This add-on does not run any application itself — it only proxies traffic to a backend server you configure via NGINX. It was inspired by and based on [Frigate's proxy add-on](https://github.com/blakeblackshear/frigate/tree/master/docker/frigate-proxy), which provides the same pattern for accessing Frigate's web UI through Home Assistant.

## Security

**Is this secure?** Yes — with important context:
